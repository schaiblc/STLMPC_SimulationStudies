...
\begin{figure}[ht]
    \centering
    \subfloat[]{\includegraphics[width=0.24\textwidth]{Figures/fig_softmin_coefficienta.png}\label{fig_softmin_coefficienta}} \hfil
  \subfloat[]{\includegraphics[width=0.24\textwidth]{Figures/fig_softmin_coefficientb.png}\label{fig_softmin_coefficientb}}
...
The planner is first characterized in the \textit{f1tenth\_simulator}, where the breadth and statistical repetition impractical on hardware isolate the contribution of individual components, assess parameter sensitivity and probe robustness; the experiments of Sections \ref{sec:exp1}--\ref{sec:exp3} then validate that this behaviour transfers to a physical vehicle. Studies use the simulator maps of Section~\ref{sec6}, with the scenario type set by the planner and adversary configuration rather than the map. The simulated plant is a single-track model with lateral tire dynamics, not the kinematic bicycle model the planner assumes internally; it engages above $0.53$~m/s and every reported run operates between $1.4$ and $2.7$~m/s, so all results below reflect plant--model mismatch, including at racing speeds approaching $v_\textup{max}$.

Each configuration is repeated over $n=10$ trials with randomized start pose ($\pm0.3$~m in position, $\pm10^{\circ}$ in heading), reported as mean~$\pm$~standard deviation. Two outcomes are distinguished: a trial is \emph{collision-free} if the vehicle never approaches an obstacle closer than $0.15$~m, and \emph{successful} if it is collision-free \emph{and} reaches the course goal within $60$~s. Performance throughout Section \ref{sec6} is measured by the minimum and average obstacle proximity over a run ($\min_{\tilde k} d_{\textup{min},\tilde k}$ and $\bar d_\textup{min}$, where $\tilde k$ indexes the control step), the control-input averages and variances ($\overline{|\delta_{\textup{cmd}}|}$, $\mathrm{Var}(\delta_{\textup{cmd}})$, $\bar v_{\textup{cmd}}$, $\mathrm{Var}(v_{\textup{cmd}})$) and the course time $t_\textup{tot}$; the simulation studies additionally report the outcome rates above and, for dynamic scenarios, the minimum time-to-collision ($\min\textup{TTC}$) with respect to the adversary. Simulation statistics aggregate the 10 randomized trials, whereas the hardware tables report a single representative run per planner.

\textit{1) Component Ablations:} Table \ref{tab:sim_ablation} isolates the two central design choices. The first two blocks vary the number of sequential tracking lines $n_\textup{MPC}$ while holding the horizon length $n_\textup{MPC}k_\textup{MPC}$ fixed, so that any difference reflects the sequential-line contribution rather than horizon length; a single line ($n_\textup{MPC}=1$) corresponds to a lengthened version of the non-predictive tracking scheme. Because the benefit of a piecewise-linear reference depends on how much of a curve the horizon spans, the comparison is repeated at two horizon lengths. The third block ablates the variable-velocity soft constraints: removing the steering-based limit \eqref{5.3.5}, removing the obstacle-proximity limit \eqref{5.3.13}, and replacing both soft limits with a hard post-hoc clamp of the commanded speed to $\min(f_{v_\textup{steer}},f_{v_\textup{obs}})$ evaluated at the current state.

The sequential-line benefit is governed by how much corridor curvature the horizon spans. At the nominal $16$-sample horizon---$1.6$~s, or $2.4$~m of travel at $v=1.5$~m/s---one line covers the corridor as well as two on these maps, with $n_\textup{MPC}=1$ marginally ahead on clearance ($0.84$ versus $0.77$~m; the same ordering holds on Map~\#1). Doubling the horizon to $4.8$~m reverses it: two lines then give the highest clearance of any configuration ($0.88$ versus $0.84$~m) while using \emph{fewer} objective evaluations per step ($20$ versus $24$), so the gain derives from the piecewise-linear reference geometry rather than from added optimization effort. Subdividing further is counterproductive---with $n_\textup{MPC}=4$ the segments are too short to constrain the reference, and at the nominal horizon only $40\%$ of trials complete. The sequential construction therefore pays off once the lookahead spans corridor curvature, the regime for which it was designed, and the required lookahead is set by course geometry rather than by a fixed horizon length.

For the velocity constraints, replacing both soft limits with the hard post-hoc clamp gives the lowest clearance of the four variants ($0.55$ versus $0.68$~m for the full method, a gap of roughly twice the combined standard deviation) at a comparable mean speed, confirming that enforcing the speed limits \emph{inside} the horizon, where the optimizer can trade them against the tracking and steering terms, is preferable to imposing them after the fact. Removing the steering-based limit likewise reduces clearance ($0.62$~m) while raising mean speed, as the vehicle is no longer slowed through the switchback's tight sections. Removing the obstacle-proximity limit produces no measurable clearance penalty on this map ($0.72$ versus $0.68$~m, within one standard deviation); its role is to govern the approach to blocked corridors, a regime this course does not exercise, and its contribution is therefore not resolved by this ablation.
...
% ==========================================================================
\begin{table}[ht]
  \caption{Simulation ablations over $n=10$ randomized trials on the switchback map (Map~\#4): sequential tracking lines at two horizon lengths (constant $v$) and velocity constraints (variable $v$)}
  ...
  \begin{tabular}{l@{\hskip 4pt}c@{\hskip 5pt}c@{\hskip 5pt}c@{\hskip 5pt}c@{\hskip 5pt}c}
    ...
    \midrule\\[-0.85em]
    \multicolumn{6}{l}{\scriptsize\textit{Horizon $n_\textup{MPC}k_\textup{MPC}\!=\!16$ ($1.6$~s, $2.4$~m at $v\!=\!1.5$~m/s)}}\\
    \scriptsize A1: single line ($n_\textup{MPC}\!=\!1$) & 100 & \ms{0.84}{0.01} & \ms{1.22}{0.00} & \ms{0.015}{0.000} & \ms{1.45}{0.00} \\
    \scriptsize A2: baseline ($n_\textup{MPC}\!=\!2$) & 100 & \ms{0.77}{0.01} & \ms{1.19}{0.00} & \ms{0.020}{0.001} & \ms{1.44}{0.00} \\
    \scriptsize A3: four lines ($n_\textup{MPC}\!=\!4$) & 40 & \ms{0.68}{0.06} & \ms{1.00}{0.13} & \ms{0.013}{0.006} & \ms{0.98}{0.38} \\
    \midrule\\[-0.85em]
    \multicolumn{6}{l}{\scriptsize\textit{Horizon $n_\textup{MPC}k_\textup{MPC}\!=\!32$ ($3.2$~s, $4.8$~m at $v\!=\!1.5$~m/s)}}\\
    \scriptsize A1$'$: single line ($n_\textup{MPC}\!=\!1$) & 100 & \ms{0.84}{0.01} & \ms{1.22}{0.00} & \ms{0.016}{0.000} & \ms{1.45}{0.00} \\
    \scriptsize A2$'$: two lines ($n_\textup{MPC}\!=\!2$) & 100 & \ms{0.88}{0.01} & \ms{1.22}{0.00} & \ms{0.015}{0.001} & \ms{1.45}{0.00} \\
    \scriptsize A3$'$: four lines ($n_\textup{MPC}\!=\!4$) & 100 & \ms{0.78}{0.05} & \ms{1.18}{0.01} & \ms{0.021}{0.001} & \ms{1.43}{0.01} \\
    \midrule\\[-0.85em]
    \scriptsize V1: full method & 100 & \ms{0.68}{0.06} & \ms{1.15}{0.01} & \ms{0.020}{0.001} & \ms{2.59}{0.02} \\
    \scriptsize V2: no $g_{v_\textup{steer}}$ & 100 & \ms{0.62}{0.06} & \ms{1.11}{0.01} & \ms{0.023}{0.001} & \ms{2.68}{0.02} \\
    \scriptsize V3: no $g_{v_\textup{obs}}$ & 100 & \ms{0.72}{0.05} & \ms{1.15}{0.01} & \ms{0.019}{0.001} & \ms{2.61}{0.01} \\
    \scriptsize V4: hard clamp & 100 & \ms{0.55}{0.03} & \ms{1.06}{0.01} & \ms{0.020}{0.001} & \ms{2.66}{0.01} \\
    \bottomrule
  \end{tabular}
\end{table}

\textit{2) Parameter Sensitivity:} Fig. \ref{fig:sensitivity} sweeps each of the principal hyperparameters one-at-a-time over a range of roughly $\times\frac{1}{3}$ to $\times3$ about its nominal value, reporting the resulting clearance bands. Four of the six parameters are essentially inert over the swept range: the gap hazard threshold $d_\textup{safe}$, softmin sharpness $\beta_v$ and velocity decay $\alpha_v$ vary $\bar d_\textup{min}$ by under $2\%$, and all trials complete. The two objective weights behave as their roles predict and vary smoothly rather than sharply: increasing $\lambda_{\dot d^2}$ from $10$ to $90$ penalizes lateral-rate deviation more heavily and reduces mean clearance from $1.22$ to $1.04$~m, while increasing the velocity weight $\lambda_{v^2}$ raises it from $1.13$ to $1.20$~m. Both are monotone and free of discontinuities, so the nominal values sit on a gradual trade-off curve rather than at a sharp optimum.

The passband steepness $s_\theta$ is the one parameter with a genuine stability boundary, and it is reported here because it bounds the admissible configuration. Values at or below the nominal $200$~$\textup{rad}^{-1}$ are indistinguishable in every metric ($\bar d_\textup{min}=1.14$~m and $100\%$ success at $s_\theta=67$, $115$ and $200$, with $49$--$56$ objective evaluations per control step). At $s_\theta=350$ and above, the passband transition becomes too steep for the gradient-based solver: the objective evaluations per step rise by nearly three orders of magnitude (to $4.0\times10^{4}$), the solver returns $J_\textup{final}=J_\textup{init}$---that is, it makes no progress from the initial guess before the $50$~ms limit---mean clearance falls to $0.65$~m, and no trial completes. This is the numerical-smoothness limit anticipated in Section \ref{sec5} made quantitative: $s_\theta$ must remain low enough that $\theta_{\text{band},i,j}$ retains usable gradients, and the nominal value lies at the upper end of that admissible interval. The effect is one of gradient conditioning rather than of the feasibility tolerance: repeating $s_\theta=350$ with the constraint tolerance varied from $10^{-2}$ to $10^{-8}$ leaves $J_\textup{final}/J_\textup{init}=1$ in every case, since at that steepness the constraint linearization underlying the SQP subproblem is valid only over a vanishing neighbourhood and no trial step is accepted. Since performance is flat for all smaller values, a more conservative $s_\theta$ would sacrifice nothing and widen the margin.
...
\begin{figure}[ht]
    ...
    \caption[One-at-a-time parameter sensitivity]{One-at-a-time parameter sensitivity of STLMPC on Map~\#4 over $n=10$ trials per point. Each panel sweeps one parameter ($\lambda_{\dot d^2}$, $\lambda_{v^2}$, $d_\textup{safe}$, $\beta_v$, $\alpha_v$, $s_\theta$) about its nominal value (dashed line); shaded bands show mean~$\pm$~std of the average clearance $\bar d_\textup{min}$. Note the differing vertical scales: $d_\textup{safe}$, $\beta_v$ and $\alpha_v$ vary by under $2\%$, the two objective weights vary smoothly and monotonically, and only $s_\theta$ exhibits a stability boundary, degrading sharply above the nominal value where the passband gradients become too steep for the solver.}
    \label{fig:sensitivity}
\end{figure}

\textit{3) Robustness and Baselines:} Table \ref{tab:sim_robust} reports harder scenarios alongside the gap-following and sampling-based baselines. Dynamic scenarios use a scripted adversary whose motion violates the constant-velocity/curvature prediction assumption through mid-encounter braking and swerving, and a two-vehicle case in which one adversary is initially occluded; sensor degradation is emulated by elevated scan noise with random beam dropout. Across the entire campaign of $650$ trials---every configuration in Tables \ref{tab:sim_ablation} and \ref{tab:sim_robust} and the sensitivity sweep of Fig. \ref{fig:sensitivity}---no trial ended in a collision; reported failures are exclusively timeouts in which the vehicle did not reach the course goal within $60$~s.

Under maneuvers that break the prediction model the minimum TTC degrades gracefully rather than collapsing, from $2.50$~s under braking to $1.91$~s under swerving and $1.97$~s in the occluded case, with all trials completing and map clearance never below $0.95$~m. Rows N1--N3 repeat these encounters with the prediction branch of Section \ref{sec4} disabled, isolating its contribution as the third modular ablation. The branch raises the minimum TTC in every encounter ($1.40\!\rightarrow\!2.50$, $1.55\!\rightarrow\!1.91$ and $1.77\!\rightarrow\!1.97$~s) and reduces its spread under braking ($\pm0.26$ versus $\pm0.66$~s), the maneuver the constant-velocity model mispredicts most severely; the smallest gain is the occluded case, where the second vehicle is not observable early enough for prediction to help. The margin is bought laterally: with the branch active the vehicle passes closer to the static map ($1.07$ versus $1.55$~m in the swerve encounter), trading static for dynamic clearance. As the adversary is not part of the occupancy grid, this ablation quantifies avoidance margin rather than crash avoidance; Section \ref{sec:exp2} provides the physical validation. Sensor degradation produces a smooth monotone penalty---mean minimum clearance $0.64\!\rightarrow\!0.51$~m as the scan noise standard deviation rises from $0.01$ to $0.05$~m with $5\%$ beam dropout---with no loss of completion.

The baselines are compared on Map~\#4, the course used throughout the ablations. On this static layout FGM is competitive on clearance, completing every trial at $0.83$~m---expected rather than surprising, since steering to the centre of the widest gap is close to clearance-maximizing in a corridor and STLMPC's heading initialization derives from the same gap construction. The distinction lies in what that clearance costs and in what the heuristic cannot express. FGM is confined to the constant speed at which it steers, $1.50$~m/s, whereas STLMPC traverses the same course at $2.58$~m/s for $0.17$~m less margin, the velocity constraints of Section \ref{sec5} allowing speed to be traded against proximity within the optimization. Gap-following offers no comparable mechanism and no means of incorporating a predicted obstacle trajectory: in the braking-adversary encounter it attains a minimum TTC of $1.35\pm0.63$~s against $2.50\pm0.26$~s for STLMPC with the prediction branch, and completes $90\%$ of trials against $100\%$. That figure is indistinguishable from STLMPC with the branch disabled ($1.40\pm0.66$~s, row N1), confirming the deficit is the absence of prediction rather than a property of the heuristic. The QP and MPC layers therefore contribute speed modulation and anticipation of moving obstacles rather than additional clearance on a static course.

Generalization is assessed on two maps not used in the ablations (Maps~\#3 and~\#5, the latter forked). Both are traversed without collision at average clearance $1.65$~m and $1.24$~m, and Map~\#3 is completed in every trial. Map~\#5 is completed in $10\%$: STLMPC being goal-free, no term in \eqref{STLMPC} biases it toward the branch holding the finish point, which is instead selected on local clearance, and the remaining trials traverse the opposite branch and return safely. Goal-reaching is therefore not an informative generalization metric for a goal-free planner on a branching course, so collision-free traversal and maintained clearance are reported instead; pairing STLMPC with a global planner, as in Section \ref{sec:exp3}, removes the ambiguity. Per-seed results are provided as supplementary material.
...
% ==========================================================================
\begin{table}[ht]
  \caption{Simulation robustness, dynamic-branch ablation and baselines (FGM gap-following; MPPI on the identical objective) over $n=10$ randomized trials}
  ...
  \begin{tabular}{l@{\hskip 4pt}c@{\hskip 5pt}c@{\hskip 5pt}c@{\hskip 5pt}c@{\hskip 5pt}c}
    ...
    \midrule\\[-0.85em]
    \scriptsize R1: adversary brakes & 100 & \ms{1.52}{0.06} & \ms{2.56}{0.02} & \ms{2.50}{0.26} & \ms{1.50}{0.00} \\
    \scriptsize R2: adversary swerves & 100 & \ms{1.07}{0.29} & \ms{2.53}{0.06} & \ms{1.91}{0.23} & \ms{1.49}{0.00} \\
    \scriptsize R3: two veh., occluded & 100 & \ms{0.95}{0.24} & \ms{2.35}{0.20} & \ms{1.97}{0.21} & \ms{1.48}{0.01} \\
    \multicolumn{6}{l}{\scriptsize\textit{Prediction branch disabled (ablation of the dynamic-obstacle branch)}}\\
    \scriptsize N1: brakes, no pred. & 100 & \ms{1.56}{0.01} & \ms{2.62}{0.01} & \ms{1.40}{0.66} & \ms{1.50}{0.00} \\
    \scriptsize N2: swerves, no pred. & 100 & \ms{1.55}{0.01} & \ms{2.61}{0.01} & \ms{1.55}{0.18} & \ms{1.50}{0.00} \\
    \scriptsize N3: two veh., no pred. & 100 & \ms{0.89}{0.27} & \ms{2.33}{0.22} & \ms{1.77}{0.21} & \ms{1.48}{0.01} \\
    \scriptsize \quad FGM, brakes & 90 & \ms{1.09}{0.06} & \ms{2.17}{0.06} & \ms{1.35}{0.63} & \ms{1.50}{0.00} \\
    \midrule\\[-0.85em]
    \scriptsize R4: scan noise $\sigma\!=\!0.01$ & 100 & \ms{0.64}{0.01} & \ms{1.72}{0.00} & --- & \ms{1.48}{0.00} \\
    \scriptsize \quad\ $\sigma\!=\!0.03$ & 100 & \ms{0.59}{0.01} & \ms{1.68}{0.00} & --- & \ms{1.48}{0.00} \\
    \scriptsize \quad\ $\sigma\!=\!0.05$ & 100 & \ms{0.51}{0.02} & \ms{1.64}{0.01} & --- & \ms{1.47}{0.00} \\
    \midrule\\[-0.85em]
    \scriptsize FGM baseline & 100 & \ms{0.83}{0.01} & \ms{1.25}{0.01} & --- & \ms{1.50}{0.00} \\
    \scriptsize MPPI, matched budget & 0 & \ms{0.40}{0.10}$^{\ddagger}$ & \ms{0.90}{0.11}$^{\ddagger}$ & --- & \ms{0.92}{0.21} \\
    \scriptsize MPPI, $3.8\times$ budget & 10 & \ms{0.48}{0.12}$^{\ddagger}$ & \ms{0.80}{0.19}$^{\ddagger}$ & --- & \ms{0.75}{0.43} \\
    \scriptsize STLMPC-SQP & 100 & \ms{0.66}{0.06}$^{\ddagger}$ & \ms{1.15}{0.02}$^{\ddagger}$ & --- & \ms{2.58}{0.02} \\
    \bottomrule\\[-0.85em]
    \multicolumn{6}{@{}p{0.97\columnwidth}@{}}{\scriptsize $^{\ddagger}$ Variable-velocity rows measure clearance against the subsampled obstacle set of Section \ref{sec5} rather than the raw scan, which overstates $\bar d_\textup{min}$ by a measured $0.048$~m and $\min_{\tilde k}d_{\textup{min},\tilde k}$ by $0.013$~m; the constant-velocity and FGM rows use the raw scan.}
  \end{tabular}
\end{table}

\textit{4) Solver Choice and Statistics:} Aggregated over the simulation trials run at the nominal parameter values, the SLSQP optimization uses a mean of $98$ objective evaluations per control step (maximum $3562$ on any single step), reaches the $50$~ms limit on $29\%$ of control steps, and reduces the objective from the Algorithm~\ref{algo2} initial guess to the returned solution by a mean ratio $\bar J_\textup{final}/\bar J_\textup{init}=0.47$. The last figure substantiates the claim that the feasible initial guess places the solver near a low-cost local optimum: rather than being merely feasible, the guess is already within roughly a factor of two of the returned objective, so the steps taken within the time budget are refinements. That the time limit is reached on a substantial minority of steps is by design and not a failure mode---the limit returns the best feasible iterate found so far, so the control rate is maintained without ever forfeiting feasibility, and no trial in the campaign ended in a collision.

To assess whether the SQP choice or its local solutions are a limiting factor, a sampling-based Model Predictive Path Integral (MPPI) optimizer is run on the \emph{identical} formulation---the same objective and the same constraints, imposed as quadratic penalties over kinematically-rolled-out samples---as a solver-only substitution on the racing map (Table \ref{tab:sim_robust}). The decisive comparison is made per control step, since a closed-loop race confounds solution quality with warm-starting, tuning and control rate. At each step the identical problem instance is also solved by MPPI from the same Algorithm~\ref{algo2} initial guess, both objectives being recorded while the SQP solution is applied. Over $2341$ matched instances MPPI improved on the SQP objective in $0.38\%$ of cases, its best objective a median factor of $3.9$ higher, while using $2048$ objective evaluations per step against SQP's $58$. With both optimizers seeing the same problem from the same starting point, this isolates solution quality: the SQP solution is not an inferior local optimum that broader sampling would improve upon, so the non-convexity of \eqref{STLMPC} is not a practical limitation.

The closed-loop rows agree. MPPI is reported at two budgets because its $2048$-rollout configuration needs $137.6$~ms per step against SQP's $36.0$~ms, falling below the $10$~Hz rate; reducing it to $512$ rollouts ($34$~ms) restores the rate at matched computation without improving the outcome, so the deficit is not a rate artifact. Per rollout MPPI is nine times cheaper than an SQP evaluation ($67$ versus $623~\mu$s, the latter including the analytical gradient)---the cost lies in needing far more of them. Parallelized implementations reach high control rates \cite{Williams2016}, so this is not a claim about sampling-based control in general; the per-step result stands independently of implementation efficiency, comparing solutions at fixed sample count rather than fixed time.
...
\begin{figure*}[!t]
  \centering
  \subfloat[]{\includegraphics[width=0.24\textwidth]{Figures/ExperimentMap1CROPPED.jpg}\label{CourseLayouts1}} \hfil
  \subfloat[]{\includegraphics[width=0.24\textwidth]{Figures/ExperimentMap2CROPPED.jpg}\label{CourseLayouts2a}} \hfil
  \subfloat[]{\includegraphics[width=0.24\textwidth]{Figures/ANON2_Experiment_Dynamic_Frame.jpg}\label{CourseLayouts2b}}
  \hfil
  \subfloat[]{\includegraphics[width=0.24\textwidth]{Figures/ExperimentMap3IMAGE.jpg}\label{CourseLayouts3}}
...
Several limitations are disclosed. First, the receding-horizon scheme carries no formal closed-loop stability or recursive-feasibility guarantee; in place of a proof, practical stability is supported empirically by the feasible warm start, the feasibility-preserving timeout, and the convergence statistics reported above. Second, the kinematic bicycle model is valid only for low tire slip and lateral acceleration \cite{Polack2017}; the steering-based velocity limit \eqref{5.3.5} acts to keep operation inside this envelope. The simulation studies quantify the residual risk, since the plant there carries lateral tire dynamics the planner does not model and all trials remained collision-free, but mismatch may still grow beyond the speeds tested. Third, adversary prediction assumes constant velocity and curvature, so accuracy degrades under sudden braking or sharp turns---the extent of this degradation is characterized by the R1/R2 robustness rows above. Fourth, the passband steepness admits an upper bound: beyond $s_\theta\approx200$~$\textup{rad}^{-1}$ the gradients of \eqref{5.3.13} become too sharp for the solver and convergence is lost, so $s_\theta$ must be chosen within the interval identified in Fig. \ref{fig:sensitivity}. Fifth, being goal-free, the planner has no mechanism for selecting between branches at a fork; where a particular branch is required, a global planner must supply that intent, as in Section \ref{sec:exp3}. Finally, detection robustness under low-light or heavy-occlusion conditions is inherited from the perception module and is outside the scope of the planner evaluated here.

\section{Conclusion}\label{sec7} %Also include future work briefly
In this article, the formulation of a novel, open-source, standalone local path planner was outlined for safe navigation in unknown surroundings. Sequential local reference tracking lines are generated at each control step through quadratic optimizations before a non-convex MPC optimization incorporates nonholonomic system constraints within trajectory prediction. Dynamic obstacles are considered through a complete detection, tracking and avoidance framework, while racing applications are accommodated through an adaptation which promotes higher speeds when safe. In unknown static, dynamic and racing environments, both in simulated and real-world tests, local planning using this novel algorithm attains improved obstacle clearance and lower control effort relative to exploration-based schemes configured for goal-free navigation. Component-wise ablations over $650$ randomized simulation trials, none of which ended in a collision, isolate the contribution of each element: the sequential tracking lines improve clearance once the prediction horizon is long enough to span curvature, the dynamic-obstacle branch raises the minimum time-to-collision against maneuvering adversaries by up to $79\%$, and the soft velocity constraints outperform an equivalent hard post-hoc clamp. The same studies delimit where each component does not contribute, and identify the numerical bound on the passband steepness beyond which the gradient-based solver ceases to converge.
